...
function mountDummyElement(): Element {
  const existingDummy = document.getElementById("aideye-dummy-element");
  if (existingDummy) {
  ...
  element.id = "aideye-dummy-element";
...
export function highlight(step: AidEyeStep) {
  const { element } = step;
  let elemObj =
...
function transferHighlight(toElement: Element, toStep: AidEyeStep) {
  const duration = 400;
  ...
  const isToDummyElement = toElement.id === "aideye-dummy-element";
  const isFromDummyElement = fromElement.id === "aideye-dummy-element";
  ...
  fromElement.classList.remove(
    "aideye-active-element",
    "aideye-no-interaction"
  );
  ...
  if (disableActiveInteraction) {
    toElement.classList.add("aideye-no-interaction");
  }

  toElement.classList.add("aideye-active-element");
  toElement.setAttribute("aria-haspopup", "dialog");
  toElement.setAttribute("aria-expanded", "true");
  toElement.setAttribute("aria-controls", "aideye-popover-content");
}

export function destroyHighlight() {
  document.getElementById("aideye-dummy-element")?.remove();
  document.querySelectorAll(".aideye-active-element").forEach((element) => {
    element.classList.remove("aideye-active-element", "aideye-no-interaction");
