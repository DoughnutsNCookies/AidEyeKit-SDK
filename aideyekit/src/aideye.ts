...
export function aideye(options: Config = {}) {
  configure(options);
  ...
  function init() {
    ...
    document.body.classList.add(
      "aideye-active",
      getConfig("animate") ? "aideye-fade" : "aideye-simple"
    );
  ...
  function destroy(withOnDestroyStartedHook = true) {
    ...
    // the hook for when user calls `destroy`, aideye will get into infinite loop
    // not causing tour to be destroyed.
    if (withOnDestroyStartedHook && onDestroyStarted) {
      const isActiveDummyElement =
        !activeElement || activeElement?.id === "aideye-dummy-element";
      onDestroyStarted(
    ...
    if (activeElement && activeStep) {
      const isActiveDummyElement = activeElement.id === "aideye-dummy-element";
      if (onDeselected) {
        onDeselected(
          isActiveDummyElement ? undefined : activeElement,
          activeStep,
...
export type AidEye = ReturnType<typeof aideye>;
